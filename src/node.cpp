...
void removeDepthless(std::vector<cv::KeyPoint>& feature_locations_2d, const cv::Mat& depth)
{
    ...
    while (i < feature_locations_2d.size()) {
        ...
        if (p2d.x >= depth.cols || p2d.x < 0 || p2d.y >= depth.rows || p2d.y < 0 || std::isnan(p2d.x) || std::isnan(p2d.y)) { //TODO: Unclear why points should be outside the image or be NaN
    ...
    , cam_info_(*cam_info)
{
    ...
    if (ps->get<int>("emm__skip_step") <= 0 && !ps->get<bool>("store_pointclouds") && ps->get<bool>("use_icp")) { //if clearing out point clouds, the icp structure needs to be built before
        gicp_mutex.lock();
        gicp_point_set_ = this->getGICPStructure();
        gicp_mutex.unlock();
    }
#endif
    if (ps->get<bool>("use_root_sift") && (ps->get<std::string>("feature_extractor_type") == "SIFTGPU" || ps->get<std::string>("feature_extractor_type") == "SURF" || ps->get<std::string>("feature_extractor_type") == "GFTT" || ps->get<std::string>("feature_extractor_type") == "SIFT")) {
    ...
    , initial_node_matches_(0)
{
    ...
    */
    ...
    gicp_initialized = false;
    gicp_point_set_ = NULL;
    if (!ps->get<bool>("store_pointclouds") && ps->get<bool>("use_icp")) { //if clearing out point clouds, the icp structure needs to be built before
        gicp_mutex.lock();
        gicp_point_set_ = this->getGICPStructure();
        gicp_mutex.unlock();
    }
    ...
    if ((!ps->get<bool>("use_glwidget") || !ps->get<bool>("use_gui")) && !ps->get<bool>("store_pointclouds") && !ps->get<int>("emm__skip_step")) {
    ...
    const Eigen::Matrix4f& initial_transformation)
{
    ...
    if (gicp_point_set->Size() > Node::gicp_min_point_cnt && target_gicp_point_set->Size() > Node::gicp_min_point_cnt) {
...
dgc::gicp::GICPPointSet* Node::getGICPStructure(unsigned int max_count) const
{
    ScopedTimer s(__FUNCTION__);
    if (max_count == 0)
        max_count = ParameterServer::instance()->get<int>("gicp_max_cloud_size");
    //Use Cache
    ...
    std::vector<float>& descriptors_in, cv::Mat& descriptors_out)
{
    ...
    for (unsigned int i = 0; i < feature_locations_2d.size(); /*increment at end of loop*/) {
        ...
        i++;                           //Only increment if no element is removed from vector
        if (feature_locations_3d.size() >= max_keyp)
            break;
    }
...
bool searchLabelInNeighborhood(const pointcloud_type::Ptr point_cloud, cv::Point2f center, float diameter, int label)
{
    ...
    left = left < 0 ? 0 : left;
    int bot = center.y + radius;
    bot = bot >= point_cloud->height ? point_cloud->height - 1 : bot;
    int right = center.x + radius;
    right = right >= point_cloud->width ? point_cloud->width - 1 : right;
    ...
    pointcloud_type::ConstPtr point_cloud)
{
    ...
    for (unsigned int i = 0; i < feature_locations_2d.size(); /*increment at end of loop*/) {
        p2d = feature_locations_2d[i].pt;
        if (p2d.x >= point_cloud->width || p2d.x < 0 || p2d.y >= point_cloud->height || p2d.y < 0 || std::isnan(p2d.x) || std::isnan(p2d.y)) { //TODO: Unclear why points should be outside the image or be NaN
            ROS_WARN_STREAM("Ignoring invalid keypoint: " << p2d);                                                                             //Does it happen at all? If not, remove this code block
            feature_locations_2d.erase(feature_locations_2d.begin() + i);
            continue;
        }
    ...
    std::vector<cv::DMatch>& matches) const
{
    ...
    std::stringstream nodesstringstream;
    nodesstringstream << "Nodes " << (this->id_) << "<->" << (earlier_node->id_);
    ...
    for (int n = 0; (n < ransac_iterations && matches_with_depth->size() >= sample_size); n++) //Without the minimum number of matches, the transformation can not be computed as usual TODO: implement monocular motion est
    {
        ...
        if (refined_matches.size() > 0) { //Valid?
            ...
            if (refined_error <= rmse && refined_matches.size() >= matches.size() && refined_matches.size() >= min_inlier_threshold) {
                ...
                //Performance hacks:
                if (refined_matches.size() > initial_matches->size() * 0.5)
                    n += 10; ///Iterations with more than half of the initial_matches inlying, count tenfold
                if (refined_matches.size() > initial_matches->size() * 0.75)
                    n += 10; ///Iterations with more than 3/4 of the initial_matches inlying, count twentyfold
                if (refined_matches.size() > initial_matches->size() * 0.8)
                    break; ///Can this get better anyhow?
            }
        }
    }                          //iterations
...
MatchingResult Node::matchNodePair(const Node* older_node)
{
    ...
    try {
        ...
        if (ParameterServer::instance()->get<int>("max_connections") > 0 && initial_node_matches_ > ParameterServer::instance()->get<int>("max_connections"))
        ...
        if (ParameterServer::instance()->get<bool>("use_icp")) {
                                                                                          ...
                                                                                          //|| (!found_transformation && initial_node_matches_ == 0)) //no matches were found, and frames are not too far apart => identity is a good initial guess.
            {
                    ...
                    !((mr_icp.icp_trafo.array() != mr_icp.icp_trafo.array()).any()))                       //No NaNs
                {
                    ...
                    if (no_obs_test || (observation_criterion_met(mr_icp.inlier_points, mr_icp.outlier_points, mr_icp.occluded_points + mr_icp.inlier_points + mr_icp.outlier_points, icp_quality) && icp_quality > ransac_quality)) {
...
#pragma omp parallel sections reduction(+ \
                                        : inlier_points, outlier_points, all_points, occluded_points)
    {
#pragma omp section
        {
